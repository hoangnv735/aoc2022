...
e_best_flow_rate = 0
e_cur_flow_rate = 0
e_potential_valves = None


def e_move(v, t):
    global e_best_flow_rate
    global e_cur_flow_rate
    global e_potential_valves
    global visited

    visited[v] = True
    if e_best_flow_rate < e_cur_flow_rate:
        e_best_flow_rate = e_cur_flow_rate
    for u in e_potential_valves:
        d = distance(v, u)
        if not visited[u] and d + 1 < t:
            new_t = t - d - 1
            e_cur_flow_rate += flow_rates[u] * new_t
            e_move(u, new_t)
            e_cur_flow_rate -= flow_rates[u] * new_t
    visited[v] = False
...
def move(v, t):
    ...
    visited[v] = True
    e_potential_valves = [v for v in potential_valves if not visited[v]]
    e_best_flow_rate = 0
    e_cur_flow_rate = 0
    
    e_move("AA", 26)
    if best_flow_rate < cur_flow_rate + e_best_flow_rate:
        best_flow_rate = cur_flow_rate + e_best_flow_rate

    for u in potential_valves:
        d = distance(v, u)
        if not visited[u] and d + 1 < t:
            new_t = t - d - 1
